...
class Wordle(ctk.CTk):
    ...
    def create_widgets(self):
        # Blank Frame
        self.blank_frame = ctk.CTkFrame(self, width = 302, height = 550, fg_color = 'white', corner_radius = 10)
        ...
        self.word_entry = ctk.CTkEntry(self, textvariable = self.word_var, font = ctk.CTkFont(size = 20), width = 95)
        self.bind('<Return>', lambda event: self.apply_word())
        self.bind('<Alt-KeyPress-a>', lambda event: self.restart_app())

        # Buttons
        self.button = ctk.CTkButton(self, text = 'Apply', font = ctk.CTkFont(size = 15), command = self.apply_word, width = 80)
        self.restart = ctk.CTkButton(self, text = 'Restart', font = ctk.CTkFont(size = 15), command = self.restart_app, width = 80)
        self.switch_widgets_button = ctk.CTkButton(self, text = '', font = ctk.CTkFont(size = 15), fg_color = 'white', hover_color = 'grey', command = self.switch_widgets, width = 30)
...
class Stats_Frame(ctk.CTkFrame):
    def __init__(self, parent, json_data = None):
        super().__init__(master = parent, width = 302, height = 550, corner_radius = 10, border_width = 3, border_color = 'white')
        self.rowconfigure(0, weight = 1, uniform = 'a')
        ...
        # Achievements Scrollable Frame
        ctk.CTkScrollableFrame(self, width = 265).grid(row = 2, column = 0, padx = 5, pady = 7)
    ...
    def update_colors(self, guess, hint):
        for i in range(5):
            match hint[i]:
                ...
                case 'X':
                    self.key_list[guess[i]].configure(fg_color = '#528D4D')
